        ...
        if paneCount == 1:
            if resampleSize == 'D':
                try:
                    ...
                    stock_values = [tuple(vals) for vals in data_reset[cols].values]
                    dates = data_reset['date_ax'].tolist()
                    volume = data['volume'].tolist()

                    a.clear()

                    candlestick_ohlc(a, stock_values, width=0.6, colorup='g', colordown='r')
                    a.xaxis_date()
                    a2.bar(dates, volume, facecolor='b')
                ...
                if forceCounter > 12:
                    try:
                        ...
                        cstick = candlestick_ohlc(a, OHLC[['MPLDates','open','high','low','close']].values, width=candlewidth, colorup='g')
                        a.set_ylabel('Price')
...
class SeaofBTCapp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        OHLCI.add_command(label= '1 Day',
                            command = lambda: changeSampleSize('D', 0.05))
        OHLCI.add_command(label= '1 Week',
                            command = lambda: changeSampleSize('W',0.08))
        OHLCI.add_command(label= '1 Month',
                            command = lambda: changeSampleSize('M',0.16))
